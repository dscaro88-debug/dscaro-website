#!/usr/bin/env python3
"""
Create the DS-DIN-005 product info card xlsx, following the 002 template
structure. Pre-fills the fields that can be objectively sourced from
the product images; leaves pricing/supplier blank for the user.
"""
import zipfile
import sys
...
def build_sheet_xml():
    ...
    r += 1
    rows.append(f'<row r="{r}">{cell(f"A{r}", "SKU: DS-DIN-005  |  Dining Solutions  |  餐饮护理")}</row>')
    r += 1
    ...
    r += 1
    rows.append(label_row(r, "产品图片来源", "本文件夹内共11张图片", "主图×5 + SKU变体×1 + 详情×5"))
    r += 1
    rows.append(f'<row r="{r}"></row>')
    r += 1
    ...
    r += 1
    rows.append(label_row(r, "产品名称(中文)", "成人可拆围兜（防水防脏）", ""))
    r += 1
    rows.append(label_row(r, "产品名称(英文)", "Detachable Adult Bib (Waterproof & Washable)", ""))
    r += 1
    rows.append(label_row(r, "产品简述(中文)",
        "TPU 防水面料可拆饭兜，魔术贴调节扣设计，3D 立体防漏集渣槽，机洗便捷。适用于养老院、护理机构、餐饮护理及大龄儿童。",
        "一句话卖点"))
    r += 1
    rows.append(label_row(r, "产品简述(英文)",
        "TPU waterproof fabric detachable bib with hook & loop adjustable buckle, 3D leak-proof crumb catcher pocket, machine washable. Ideal for nursing homes, care facilities, and elderly/older children.",
    ...
    r += 1
    rows.append(label_row(r, "产品尺寸",
        "M 码: 56×36cm / L 码: 64×36cm / XL 码: 72×36cm (高 19cm 含集渣袋)",
        "长×宽 (从详情图读取)"))
    r += 1
    rows.append(label_row(r, "材质(面料)", "TPU 防水面料 + 聚酯纤维底衬", ""))
    r += 1
    rows.append(label_row(r, "单件重量", "", "Net weight per unit ⚠ 待供应商"))
    r += 1
    rows.append(label_row(r, "闭合方式", "12CM 魔术贴调节扣 (Hook & Loop Adjustable Buckle)", ""))
    r += 1
    rows.append(label_row(r, "接食槽设计", "可拆卸立体集渣袋 (Detachable 3D Crumb Catcher)", ""))
    r += 1
    rows.append(label_row(r, "洗涤方式", "可水洗/可擦/机洗便捷", ""))
    r += 1
    rows.append(label_row(r, "吸水性", "TPU 防水防污 (Waterproof & Stain-Resistant)", ""))
    r += 1
    rows.append(label_row(r, "耐温范围", "", "Care instructions ⚠ 待供应商"))
    r += 1
    ...
    r += 1
    variants = [
        ("DS-DIN-005-01", "粉色 L 码 (Pink L)", "SKU_03_粉色L码.jpg"),
    ]
    ...
    rows.append(label_row(r, "Product Title",
        "DS CARO Detachable Adult Bib Waterproof TPU Fabric Adjustable Hook Loop Crumb Catcher Washable Nursing Home Care Elderly",
        "≤128字符"))
    r += 1
    rows.append(label_row(r, "Keyword 1", "Detachable Adult Bib Waterproof", ""))
    r += 1
    rows.append(label_row(r, "Keyword 2", "TPU Bib Elderly Care", ""))
    r += 1
    rows.append(label_row(r, "Keyword 3", "Nursing Home Dining Supplies", ""))
    r += 1
    rows.append(label_row(r, "Keyword 4", "Adjustable Hook Loop Bib", ""))
    r += 1
    ...
    for label, note in [
        ("CE认证", "非医疗器械"),
        ("REACH合规", "欧盟化学品法规（TPU 材料）"),
        ("OEKO-TEX", "纺织品安全认证（加分项）"),
        ("面料测试报告", "TPU 成分测试报告增加信任度"),
    ]:
    ...
    notes = [
        "□ 【待填写】= 需联系供应商或实测后补充",
        "□ 本文件夹现有11张图片（主图×5 + 颜色SKU×1 + 详情×5），已替换 Dilidala logo 为 DSCARO 并翻译中文为英文",
        "□ 上传阿里时建议用 主图_01.jpg 作为首图",
        "□ 颜色/尺码变体在阿里后台创建为不同SKU选项（M/L/XL 三个尺码）",
        "□ 供应商 / 价格 / 包装尺寸 等信息 ⚠ 待供应商确认",
        "□ 已填字段基于公开图片视觉信息；具体以供应商确认为准",
        "□ 注意：每张图都有 \"EUROB SWITCH...\" 乱码英文水印（供应商图原有问题），已保留不处理",
    ]
    for n in notes:
...
def main():
    if len(sys.argv) != 2:
        print("usage: make_din005_xlsx.py <output_path>")
        sys.exit(1)
    out = sys.argv[1]
